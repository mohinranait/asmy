                                                            ...
                                                            <div class="modal-dialog modal-sm" role="document">
                                                                <div class="modal-content">
                                                                    <div class="modal-header">
                                                                        <h6 class="modal-title" id="exampleModalLabel">{{$key->name}} Category Delete?</h6>
...
<script>
    $(".toggle-class").on('change', function(e){
        ...
        $.ajax({
            type:"GET",
            dataType:"JSON",
            url:"{{route('primary.category.status')}}",
            data:{
...
<script>
    $(document).ready(function(){
        $("#primaryCategoryFind").on('keyup' , function(e){
            e.preventDefault();
            let category_id = $("#primaryCategoryFind").val();
            // console.log(category_id);
            $.ajax({
                method:"GET",
                url: "{{route('primary/category/keyup')}}",
                data:{category_id : category_id},
                success:function(res){
                    $('.primary-cat-table').html(res);
                    if( res.status == "nothing"){
                        $('.primary-cat-table').html("<div class=' alert alert-info text-center'>"+ 'No Data found' + " </div>")
                    }
                }
            })
        })
